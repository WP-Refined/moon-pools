...
import TrendChartCurve from './TrendChartCurve.vue';
    ...
    computed: {
        ...
        boundary() {
            ...
            const boundary = {
                ...
                maxY:
                    height - paddingObject.bottom - labelsOverflowObject.bottom,
            };
            return boundary;
        },
        params() {
            ...
            this.datasets.forEach(dataset => {
                if (Array.isArray(dataset?.data) === false) return;

                let dataArr = dataset.data.map(
                    item => (typeof item === 'number' ? item : item.downloads), // TODO: Make more generic field
                );
            ...
            if (this.max !== null && this.max > maxValue) maxValue = this.max;
            if (this.min !== null && this.min < minValue) minValue = this.min;
    ...
    methods: {
        ...
        fitLabels() {
            ...
            if (
                ...
                const chartLabelsClientRect =
                    chartLabels.$el.getBoundingClientRect();
        ...
        mouseMove(e) {
            if (this.$refs.chart !== undefined) {
                ...
                this.activeLine = this.getNearestCoordinate(
                    e.clientX - rect.left,
                );
            }
        },
